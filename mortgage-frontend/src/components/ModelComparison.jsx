...
const ModelComparison = () => {
  ...
  const getDecisionBadge = (decision) => {
    const config = {
      ...
      conditional: { class: 'badge-warning', label: 'Conditional' }
    };
    const style = config[decision?.toLowerCase()] || config.conditional;
  ...
  const getRiskBadge = (risk) => {
    ...
    return <span className={`font-medium ${colors[risk?.toLowerCase()] || colors.medium}`}>{risk?.toUpperCase()}</span>;
  };
    ...
    <div className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
              ...
              {Object.entries(metrics).map(([name, m]) => {
                ...
                return (
                  <tr key={name} className="border-b border-white/5">
                    <td className={`py-4 font-medium ${isWinner ? 'text-amber-400' : 'text-white'}`}>
                      {name} {isWinner && <span className="ml-2 text-xs">(Winner)</span>}
                    </td>
      ...
      <div className="glass-card rounded-2xl p-8 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="space-y-2">
            <label className="text-slate-300 text-sm">Payment History: {(applicant.payment_history_score * 100).toFixed(0)}%</label>
