...
MODEL_FEATURES = [
    ...
    "num_derogatory_marks",      # Severe delinquency: 90+ days late (count)
    "credit_utilization",        # Credit utilization ratio (0-1)
    "late_payment_severity_score",  # Severity-weighted payment composite (0=worst, 1=perfect)
    "home_ownership",            # Encoded: 0=rent, 1=own, 2=mortgage
    "purpose_encoded",           # Loan purpose encoded
    "num_late_payments",         # Mild delinquency: 30-59 days late (count)
...
def engineer_features_to_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Delinquency signals — three genuinely distinct features ──────────────────
    # Source columns (Kaggle):
    #   NumberOfTime30-59DaysPastDueNotWorse  → mild delinquency
    #   NumberOfTime60-89DaysPastDueNotWorse  → moderate delinquency
    #   NumberOfTimes90DaysLate               → severe delinquency (credit-reporting threshold)
    #
    # OLD (broken): all three features were identical functions of total_late.
    # NEW: each maps to a DIFFERENT source column or a severity-weighted composite.

    late_30 = df['NumberOfTime30-59DaysPastDueNotWorse']
    late_60 = df['NumberOfTime60-89DaysPastDueNotWorse']
    late_90 = df['NumberOfTimes90DaysLate']

    # 8. num_derogatory_marks: Severe delinquency ONLY (90+ days)
    #    Credit agencies report 90+ day lates as formal derogatory marks.
    features['num_derogatory_marks'] = late_90.astype(int)
    ...
    # 10. late_payment_severity_score: Severity-weighted composite across all buckets
    #     Weights reflect credit-reporting severity (90-day events penalised 3× vs 30-day).
    #     Score = 1 (perfect) → 0 (most severe). Distinct from both num_derogatory_marks
    #     (90-day count only) and num_late_payments (30-59 day count only).
    weighted_severity = (0.33 * late_30 + 0.66 * late_60 + 1.0 * late_90) / 12.0
    features['late_payment_severity_score'] = (1.0 - weighted_severity).clip(0.0, 1.0)
    ...
    # 13. num_late_payments: Mild delinquency ONLY (30-59 days)
    #     Distinct from num_derogatory_marks (90-day) and late_payment_severity_score (composite).
    features['num_late_payments'] = late_30.astype(int)
...
def split_and_balance(
    ...
    real_train_df = pd.DataFrame(X_train, columns=MODEL_FEATURES)
    real_train_df['target'] = y_train
    ...
    return train_df, real_train_df, val_df, test_df
...
def run_pipeline(data_dir: str = "data", output_dir: str = "data"):
    ...
    train_df, real_train_df, val_df, test_df = split_and_balance(features_df)
    ...
    real_train_df.to_csv(output_path / "real_train.csv", index=False)
    val_df.to_csv(output_path / "val.csv", index=False)
    test_df.to_csv(output_path / "test.csv", index=False)
    logger.info(f"Saved: train.csv ({len(train_df):,} rows, SMOTE-balanced)")
    logger.info(f"Saved: real_train.csv ({len(real_train_df):,} rows, real un-augmented)")
